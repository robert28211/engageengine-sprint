...
async function handleAPI(path, request, env) {
  ...
  // GET /api/clients/:id — client detail with jobs and tasks
  if (path.match(/^\/api\/clients\/[^/]+$/)) {
    const clientId = path.split('/')[3];

    const client = await env.DB.prepare(
      'SELECT * FROM sprint_clients WHERE id = ?'
    ).bind(clientId).first();
  ...
  if (path.match(/^\/api\/tasks\/[^/]+\/complete$/) && request.method === 'POST') {
    ...
    await env.DB.prepare(
      "UPDATE sprint_tasks SET status = 'Complete', completed_date = ? WHERE id = ?"
    ).bind(now, taskId).run();
  ...
  if (path.match(/^\/api\/tasks\/[^/]+\/reopen$/) && request.method === 'POST') {
    ...
    await env.DB.prepare(
      "UPDATE sprint_tasks SET status = 'Not Started', completed_date = '' WHERE id = ?"
    ).bind(taskId).run();
  ...
  if (path === '/api/jobs' && request.method === 'POST') {
    ...
    await env.DB.prepare(`
      INSERT INTO sprint_jobs (id, client_id, name, status)
      VALUES (?, ?, ?, 'Active')
    `).bind(id, body.client_id, body.name).run();
  ...
  if (path === '/api/tasks' && request.method === 'POST') {
    ...
    await env.DB.prepare(`
      INSERT INTO sprint_tasks (id, job_id, client_id, task_id, notes, status, assigned_to, due_date, completed_date)
      VALUES (?, ?, ?, ?, ?, 'Not Started', ?, ?, '')
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .header-stats .stat-val { color: var(--text); font-weight: 500; }
  ...
  .client-meta .jobs { color: var(--accent); }
  .client-meta .open { color: var(--amber); }
  .client-meta .done { color: var(--green); }
  ...
  .job-badge.active { background: var(--green-bg); color: var(--green); }
  .job-badge.complete { background: var(--surface2); color: var(--text-dim); }
  ...
  .task-notes { flex: 1; }
  .task-notes.completed { text-decoration: line-through; color: var(--text-dim); }
...
function renderDashboard() {
  const { clients, stats, team } = dashboardData;

  document.getElementById('headerStats').innerHTML =
    '<div><span class="stat-val">' + stats.active_clients + '</span> clients</div>' +
    '<div><span class="stat-val">' + stats.active_jobs + '</span> jobs</div>' +
    '<div><span class="stat-val">' + stats.open_tasks + '</span> open</div>';
  ...
  document.getElementById('teamRow').innerHTML = team.map(t =>
    '<div class="team-card" onclick="loadTeam(\\'' + encodeURIComponent(t.name) + '\\')">' +
      '<div class="team-avatar">' + t.name.charAt(0).toUpperCase() + '</div>' +
      '<div><div class="team-name">' + esc(t.name) + '</div>' +
      '<div class="team-tasks">' + t.assigned_tasks + ' tasks</div></div>' +
    '</div>'
  ).join('');

  const activeClients = clients.filter(c => c.active_jobs > 0 || c.open_tasks > 0);
  const inactiveClients = clients.filter(c => c.active_jobs === 0 && c.open_tasks === 0);

  document.getElementById('clientGrid').innerHTML =
    activeClients.map(clientCard).join('') +
    (inactiveClients.length ? '<div class="section-title" style="grid-column:1/-1;margin-top:16px">No Active Work</div>' : '') +
    inactiveClients.map(clientCard).join('');
...
function clientCard(c) {
  return '<div class="client-card" onclick="loadClient(\\'' + c.id + '\\')">' +
    '<div class="client-name">' + esc(c.name) + '</div>' +
    '<div class="client-meta">' +
      '<span class="jobs">' + c.active_jobs + ' jobs</span>' +
      '<span class="open">' + c.open_tasks + ' open</span>' +
      '<span class="done">' + c.done_tasks + ' done</span>' +
    '</div></div>';
}
...
function renderClientDetail(data) {
  const { client, jobs, tasks } = data;
  const today = new Date().toISOString().split('T')[0];

  let html = '<div class="detail-header"><h2>' + esc(client.name) + '</h2>' +
    '<div class="sub">' + jobs.length + ' jobs · ' +
    tasks.filter(t => t.status !== 'Complete').length + ' open tasks</div></div>';
  ...
  activeJobs.forEach(job => {
    ...
    html += renderJobSection(job, jobTasks, client.id, today);
  });

  // Add Job button
  html += '<button class="add-btn" onclick="toggleAddJob(\\'' + client.id + '\\')">+ Add Job</button>';
  html += '<div id="addJobForm-' + client.id + '" class="inline-form hidden">' +
    '<input type="text" id="newJobName-' + client.id + '" placeholder="Job name...">' +
    '<button onclick="createJob(\\'' + client.id + '\\')">Add</button></div>';

  if (completeJobs.length) {
    html += '<div class="section-title" style="margin-top:24px">Completed Jobs</div>';
    completeJobs.forEach(job => {
      const jobTasks = tasks.filter(t => t.job_id === job.id);
      html += renderJobSection(job, jobTasks, client.id, today, true);
    });
  }
...
function renderJobSection(job, tasks, clientId, today, collapsed = false) {
  let html = '<div class="job-section">' +
    '<div class="job-header' + (collapsed ? ' collapsed' : '') + '" onclick="this.classList.toggle(\\'collapsed\\');this.nextElementSibling.classList.toggle(\\'hidden\\')">' +
      '<div class="job-title">' + esc(job.name) +
        ' <span style="color:var(--text-dim);font-weight:400;font-size:12px">' +
        job.open_tasks + '/' + job.total_tasks + '</span></div>' +
      '<span class="job-badge ' + (job.status === 'Active' ? 'active' : 'complete') + '">' +
        job.status + '</span>' +
    '</div>';
  ...
  tasks.forEach(t => {
    ...
    const dueClass = !isDone && t.due_date ? (t.due_date < today ? 'overdue' : (daysDiff(today, t.due_date) <= 3 ? 'soon' : '')) : '';

    html += '<div class="task-item">' +
      '<div class="task-check' + (isDone ? ' done' : '') + '" onclick="toggleTask(\\'' + t.id + '\\', ' + (isDone ? 'false' : 'true') + ', \\'' + clientId + '\\')"></div>' +
      '<div class="task-notes' + (isDone ? ' completed' : '') + '">' + esc(t.notes || 'Untitled task') + '</div>' +
      (t.assigned_to ? '<span class="task-assignee">' + esc(t.assigned_to) + '</span>' : '') +
      (t.due_date ? '<span class="task-due ' + dueClass + '">' + formatDate(t.due_date) + '</span>' : '') +
    '</div>';
  });

  // Add task inline
  html += '<div class="task-item" style="padding:6px 16px">' +
    '<button class="add-btn" style="margin:0;padding:4px 12px;font-size:12px" ' +
    'onclick="toggleAddTask(\\'' + job.id + '\\')">+ task</button></div>';

  html += '<div id="addTaskForm-' + job.id + '" class="inline-form hidden" style="padding:8px 16px">' +
    '<input type="text" id="newTaskNotes-' + job.id + '" placeholder="Task description...">' +
    '<select id="newTaskAssign-' + job.id + '"><option value="">Assign</option>' +
    '<option>Robbie</option><option>Aaron</option><option>Aiden</option><option>Burt</option></select>' +
    '<input type="date" id="newTaskDue-' + job.id + '" style="width:140px">' +
    '<button onclick="createTask(\\'' + job.id + '\\', \\'' + clientId + '\\')">Add</button></div>';
...
async function loadTeam(encodedName) {
  ...
  const res = await fetch(API + '/api/team/' + encodedName);
  const data = await res.json();
  const today = new Date().toISOString().split('T')[0];

  let html = '<div class="detail-header"><h2>' + esc(data.name) + '</h2>' +
    '<div class="sub">' + data.tasks.length + ' assigned tasks</div></div>';
  ...
  Object.keys(byClient).sort().forEach(clientName => {
    html += '<div class="section-title" style="margin-top:16px">' + esc(clientName) + '</div>';
    ...
    byClient[clientName].forEach(t => {
      const dueClass = t.due_date ? (t.due_date < today ? 'overdue' : (daysDiff(today, t.due_date) <= 3 ? 'soon' : '')) : '';
      html += '<div class="task-item">' +
        '<div class="task-check" onclick="toggleTask(\\'' + t.id + '\\', true, null, true, \\'' + encodedName + '\\')"></div>' +
        '<div class="task-notes">' + esc(t.notes || 'Untitled') + '</div>' +
        '<span class="task-assignee">' + esc(t.job_name || '') + '</span>' +
        (t.due_date ? '<span class="task-due ' + dueClass + '">' + formatDate(t.due_date) + '</span>' : '') +
      '</div>';
    });
    html += '</div>';
...
async function toggleTask(taskId, complete, clientId, isTeamView, teamName) {
  const endpoint = complete ? '/api/tasks/' + taskId + '/complete' : '/api/tasks/' + taskId + '/reopen';
  ...
  fetch(API + '/api/dashboard').then(r => r.json()).then(d => { dashboardData = d; });
}

function toggleAddJob(clientId) {
  document.getElementById('addJobForm-' + clientId).classList.toggle('hidden');
  const input = document.getElementById('newJobName-' + clientId);
...
async function createJob(clientId) {
  ...
  await fetch(API + '/api/jobs', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ client_id: clientId, name: input.value.trim() })
  });
  loadClient(clientId);
}

function toggleAddTask(jobId) {
  document.getElementById('addTaskForm-' + jobId).classList.toggle('hidden');
  const input = document.getElementById('newTaskNotes-' + jobId);
  if (input) input.focus();
}

async function createTask(jobId, clientId) {
  const notes = document.getElementById('newTaskNotes-' + jobId);
  const assign = document.getElementById('newTaskAssign-' + jobId);
  const due = document.getElementById('newTaskDue-' + jobId);
